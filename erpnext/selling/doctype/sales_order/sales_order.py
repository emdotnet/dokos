...
class SalesOrder(SellingController):
	...
	def set_delivery_date_for_item_bookings(self):
		for d in self.get("items"):
			if d.get("item_booking") and not d.get("delivery_date"):
				d.delivery_date = getdate(
					frappe.db.get_value("Item Booking", d.get("item_booking"), "starts_on")
				)
	...
	def set_indicator(self):
		...
		if self.per_billed < 100 and self.status != "Closed":
			...
			self.per_billed == 100
			and (self.per_delivered < 100 and not self.skip_delivery_note)
	...
	def on_recurring(self, reference_doc, auto_repeat_doc):
		def _get_delivery_date(ref_doc_delivery_date, red_doc_transaction_date, transaction_date):
			delivery_date = AutoRepeatScheduler(auto_repeat_doc).get_next_scheduled_date()
	...
	def update_item_bookings(self):
		...
		for d in self.get("items"):
			if d.get("item_booking"):
				...
				if not confirm_after_payment or self.grand_total == frappe.db.get_value(
					self.doctype, self.name, "advance_paid"
				):
					status = "Confirmed"
				else:
	...
	def set_skip_delivery_note(self):
		if self.order_type == "Shopping Cart" and not self.skip_delivery_note:
			for d in self.get("items"):
				if not d.get("item_booking") and not frappe.get_cached_value(
					"Item", d.item_code, "enable_item_booking"
				):
					template = frappe.get_cached_value("Item", d.item_code, "subscription_template")
					if not template or not frappe.get_cached_value(
						"Subscription Template", template, "skip_delivery_note"
					):
						return
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"Sales Order Item": {
				...
				and not frappe.db.exists("Product Bundle", doc.item_code)
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Sales Order Item": {
				...
				"field_map": {"name": "so_detail", "parent": "sales_order", "delivered_qty": "delivered_qty"},
				"postprocess": update_item,
