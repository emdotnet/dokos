...
class Customer(TransactionBase):
	...
	def create_primary_contact(self):
		if (
			not self.customer_primary_contact and not self.lead_name and self.customer_type == "Individual"
		):
	...
	def validate_primary_contact(self):
		primary_contacts = frappe.get_all(
			"Dynamic Link",
			filters={"parenttype": "Contact", "link_doctype": "Customer", "link_name": self.name},
		...
		if len(primary_contacts) > 1:
			for contact in primary_contacts:
				...
				if contact.name != self.customer_primary_contact and cint(is_primary_contact) == 1:
	...
	def has_active_subscription(self) -> bool:
		...
		return bool(
			frappe.db.exists(
				"Subscription", dict(status=("not in", ("Cancelled", "Trial")), customer=self.name)
			)
		)
