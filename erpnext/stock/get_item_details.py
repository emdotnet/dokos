...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if args.get("manufacturer"):
		...
		out["manufacturer"], out["manufacturer_part_no"] = frappe.get_value(
			"Item", item.name, ["default_item_manufacturer", "default_manufacturer_part_no"]
		)
...
def get_item_tax_info(
	company, tax_category, item_codes, item_rates=None, item_tax_templates=None, doctype=None
):
	out = {}
...
def _get_item_tax_template(args, taxes, out=None, for_validate=False):
	...
	for tax in taxes:
		tax_company, applicable_for = frappe.get_value(
			"Item Tax Template", tax.item_tax_template, ["company", "applicable_for"]
		)
		transaction_type = (
			"Sales"
			if args.get("doctype") in sales_doctypes
			else ("Purchases" if args.get("doctype") in purchase_doctypes else None)
		)
	...
	if taxes_with_validity:
		taxes = (
			sorted(taxes_with_validity, key=lambda i: i.valid_from or tax.maximum_net_rate, reverse=True)
			+ taxes_with_no_validity
		)
	else:
...
def check_packing_list(price_list_rate_name, desired_qty, item_code):
	...
	if item_price.packing_unit:
		packing_increment = flt(desired_qty) % flt(item_price.packing_unit)
