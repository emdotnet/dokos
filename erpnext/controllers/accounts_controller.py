...
class AccountsController(TransactionBase):
	...
	def before_print(self, settings=None):
		if self.doctype in [
			...
			if self.get("discount_amount") and hasattr(self, "taxes") and not df.print_hide:
				df.set("print_hide", 0)
			...
			if self.company and not self.get("taxes_and_charges"):
				if self.tax_category:
					self.taxes_and_charges = frappe.db.get_value(
						tax_master_doctype, {"tax_category": self.tax_category, "company": self.company}
					)
		...
		if advance_entries:
			...
			for d in advance_entries:
				if not advance_entries_against_si or d.reference_name not in advance_entries_against_si:
					frappe.msgprint(
						_(
							"Payment Entry {0} is linked against {1} {2}, check if it should be pulled as advance in this invoice."
		...
		if role_allowed_to_over_bill in user_roles and total_overbilled_amt > 0.1:
			frappe.msgprint(
				_("Overbilling of {} ignored because you have {} role.").format(
					total_overbilled_amt, _(role_allowed_to_over_bill)
				),
				indicator="orange",
				alert=True,
			)
...
def get_default_taxes_and_charges(
	master_doctype, tax_template=None, company=None, tax_category=None
):
	if not company:
		return {}
	...
	if tax_category:
		default_tax = frappe.db.get_value(
			master_doctype, {"tax_category": tax_category, "company": company}
		)
		...
		try:
			...
			frappe.throw(
				_("You do not have permissions to {} items in a {}.").format(
					_(actions[perm_type]), _(parent_doctype)
				),
		...
		if not transitions:
			frappe.throw(
				_("You are not allowed to update as per the conditions set in {} Workflow.").format(
					get_link_to_form("Workflow", workflow, _("Workflow"))
				),
				title=_("Insufficient Permissions"),
			)
