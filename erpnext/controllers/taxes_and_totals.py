...
class calculate_taxes_and_totals(object):
	...
	def calculate_taxes(self):
		...
		for tax in self.doc.get("taxes"):
			if tax.charge_type in ("On Previous Row Amount", "On Previous Row Total"):
				...
				is_reference_an_item_tax = (
					reference_row.charge_type == "On Net Total" and reference_row.tax_rate == 0.0
				)
				if is_reference_an_item_tax:
					continue
	...
	def get_current_tax_amount(self, item, tax, item_tax_map):
		...
		def _is_row_an_item_tax(row):
			return row.charge_type == "On Net Total" and row.tax_rate == 0.0
		...
		if item_tax_map:
			for item_code, tax_data in item_tax_map.items():
				...
				itemised_tax[item_code][tax.description] = frappe._dict(
					dict(tax_rate=tax_rate, tax_amount=tax_amount, add_deduct_tax=tax.get("add_deduct_tax"))
				)
