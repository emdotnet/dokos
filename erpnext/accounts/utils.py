...
class DuplicateAccountNumberError(frappe.ValidationError):
	pass
...
def update_reference_in_payment_entry(
	...
	if d.voucher_detail_no:
		...
		if payment_entry.down_payment:
			frappe.db.set_value(
				original_row.reference_doctype,
				original_row.reference_name,
				"down_payment_against",
				reference_details.get("reference_name"),
			)
		...
		if doctype_with_same_number:
			frappe.throw(
				_("{0} Number {1} is already used in {2} {3}").format(
					_(doctype_name), number_value, _(doctype_name).lower(), doctype_with_same_number
				),
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
	...
	if (
		...
		ref_doc.outstanding_amount = flt(outstanding["outstanding_in_account_currency"])
		frappe.db.set_value(
			voucher_type,
			voucher_no,
			"outstanding_amount",
			flt(outstanding["outstanding_in_account_currency"]),
		)
