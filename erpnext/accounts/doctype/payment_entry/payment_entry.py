...
class PaymentEntry(AccountsController):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				frappe.throw(
					_("Reference Doctype must be one of {0}").format(
						comma_or([_(x) for x in valid_reference_doctypes])
					)
				)
				...
				if d.allocated_amount:
					remarks.append(
						_("Amount {0} {1} against {2} {3}").format(
							self.party_account_currency, d.allocated_amount, _(d.reference_doctype), d.reference_name
						)
					)
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		...
		make_gl_entries(
			...
			cancel=cancel,
			adv_adj=adv_adj,
			merge_entries=(self.payment_type != "Internal Transfer"),
		)
	...
	if reference_doctype == "Dunning":
		...
		if not total_amount:
			if party_account_currency == company_currency:
				...
				otal_amount = ref_doc.get("base_grand_total") or ref_doc.get("grand_total")
				exchange_rate = 1
			else:
...
def set_party_account(dt, dn, doc, party_type):
	if dt == "Sales Invoice":
		...
		party_account = get_party_account(
			party_type, doc.get(party_type.lower()), doc.company, getattr(doc, "down_payment", None)
		)
