...
frappe.ui.form.on('Payment Entry', {
	onload: function(frm) {
		frm.ignore_doctypes_on_cancel_all = ['Sales Invoice', 'Purchase Invoice', 'Archived Document', 'Repost Payment Ledger'];
	...
	get_outstanding_invoices_or_orders: function(frm, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		if (get_outstanding_invoices) {
			btn_text = __("Get Outstanding Invoices");
		}
		else if (get_orders_to_be_billed) {
			btn_text = __("Get Outstanding Orders");
		}
	...
	validate_filters_data: function(frm, filters) {
		...
		for (let key in fields) {
			...
			if (filters[from_field] && !filters[to_field]) {
				const fmt = [to_field.replace(/_/g, " ")];
				frappe.throw(
					__("Error: {0} is mandatory field", fmt)
				);
			} else if (filters[from_field] && filters[from_field] > filters[to_field]) {
				const fmt = [key, from_field.replace(/_/g, " "), to_field.replace(/_/g, " ")];
				frappe.throw(
					__("{0}: {1} must be less than {2}", fmt)
				);
			}
		}
	},
