...
class GLEntry(Document):
	...
	def check_accounting_journal(self):
		if not self.accounting_journal and cint(
			...
			frappe.throw(
				_(
					"An accounting journal could not be found for this transaction. Please configure it and try to submit again."
				)
			)
	...
	if not adv_adj and account:
		...
		if balance_must_be:
			...
			if (balance_must_be == "Debit" and flt(balance) < 0) or (
				...
				frappe.throw(
					_("Balance for Account {0} must always be {1}").format(account, _(balance_must_be).lower())
				)
